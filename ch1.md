# 第一章：Reliable, Scalable, and Maintainable Applications
...
現今很多 applications 都是 **data-intensive**，而非 **compute-intensive**。因此 CPU 很少成為這類 applications 的瓶頸，更大的問題通常來自資料量、資料複雜性、以及資料的變更速度。

一個 data-intensive application 通常是由提供通用功能的 standard building blocks 建構而成。例如，許多 applications 都需要：

- 儲存資料，以便自己或其他 application 之後能再次找到 （***database***）
- 記住 expensive operation 的結果，加快讀取速度（***cache***）
- 允許使用者按關鍵字搜尋資料，或以各種方式對資料進行過濾（***search indexes***）
- 向其他 process 傳送訊息，進行非同步處理（***stream processing***）
- 定期處理累積的大量資料（***batch processing***）

如果這些功能聽起來平淡無奇，那是因為這些 **data system** 是非常成功的抽象：我們一直不假思索地使用它們並習以為常。絕大多數工程師不會幻想從頭開始寫 data storage engine，因為對開發 application 來說，database 已經是夠完美的工具了。

但現實沒有這麼簡單。不同的 applications 有不同的需求，因而 database systems 也是百花齊放，有著各式各樣的特性。實作 cache 有很多種手段，建立 search indexes 也有好幾種方法，諸如此類。因此在開發 application 前，我們依然有必要先弄清楚最適合手上工作的工具和方法。而且當單一工具解決不了你的問題時，組合使用這些工具可能還是有些難度的。

本書是一趟關於 data systems 的原理和實作的旅程，並講述了設計 data-intensive applications 的方法。我們將探索不同工具之間的共通性與特性，以及各自的實作原理。

本章將從我們所要實作的基礎目標開始：reliable、scalable 和 maintainable 的 data systems。我們將搞清楚這些名詞的意思，列出思考這些事情的方法。並回顧一些後續章節所需要的基礎知識。在接下來的章節中我們將抽絲剝繭，研究設計 data-intensive application 時可能遇到的 design decisions。

## Thinking About Data Systems

我們通常認為 databases、queues、caches 等工具屬於差異很大的類別。雖然 databases 和 message queue 表面上有一些相似性——都需要將資料儲存一段時間——但它們有很不一樣的 access patterns，這意味著不同的 performance characteristics 和 implementations。

那我們為什麼要把這些東西放在 *data systems* 的大傘下混為一談呢？

這幾年出現許多新的資料儲存和處理工具。它們針對不同應用場景進行最佳化，因此不再適合生硬地歸入傳統類別【1】。類別之間的界限變得越來越模糊，例如：datastores 可以被當成 message queues 用（Redis），message queues 則帶有 database-like durability guar‐ antees（Apache Kafka）。

其次，越來越多的應用程式有著各種嚴格而廣泛的要求，單一工具不足以滿足所有的資料處理和儲存需求。取而代之的是，整體工作被拆分成一系列能被單一工具有效率完成的任務，並透過 application code 將它們縫合起來。

例如，如果你將 application-managed caching layer（使用 Memcached 或類似產品）或著 full-text search server（例如 Elasticsearch 或 Solr）從 main database 分離出來，那麼讓 caches 和 indexes 和 main database 保持同步通常是 application code 的責任。[圖1-1](img/fig1-1.png) 給出了這種架構可能的樣子（細節將在後面的章節中詳細介紹）。

![](img/fig1-1.png)

**圖1-1 結合多個元件的 data system 的一種可能的架構**

當你將多個工具組合在一起提供服務時，服務的 interface 或 application programming interface (API) 通常對 clients 隱藏這些實作細節。現在，你基本上已經使用較小的通用元件建立了一個全新的、專用的 data system。這個新的複合資料系統可能會提供特定的保證，例如：cache 在寫入時會作廢或更新，使得外部 clients 能得到一致的結果。現在你不僅是 application developer，還是 data system designer 了。

設計 data system 或服務時可能會遇到很多棘手的問題，例如：當系統出問題時，如何確保資料的正確性和完整性？當部分系統 degraded 時，如何為客戶提供始終如一的良好效能？當 load 增加時，如何 scale？什麼樣的 API 才是好的 API？

影響 data system 設計的因素很多，包括參與者的技術和經驗、legacy system dependencies、交件的時間範圍、公司的風險容忍度、regulatory constraints 等，這些因素很大程度上都要看狀況。

本書著重在大多數 software systems 中很重要的三個問題：

***Reliability***

系統應該要在**逆境（adversity）**（硬體故障、軟體故障、人為錯誤）中仍可正常工作（執行正確的功能，並能達到期望的效能水準）。請參閱 [[#Reliability]]。

***Scalability***

存在合理的辦法來處理系統的成長（data volume、traffic volume、complexity）。請參閱 [[#Scalability]]。

***Maintainability***

許多不同的人（工程師、營運）在不同的生命週期，都能有效率地在系統上工作（使系統保持現有行為，並適應新的 use cases）。請參閱 [[#Maintainability]]。

人們經常追求這些詞彙，卻沒有清楚理解它們到底意味著什麼。為了工程的嚴謹性，本章的剩餘部分將探討 reliability、scalability、maintainability 的含義。為實現這些目標而使用的各種技術，架構和演算法將在後續的章節中研究。
...
如果所有這些在一起意味著“正確工作”，那麼可以把 reliability 粗略理解為「即使出現問題，也能繼續正確工作」。

造成錯誤的原因叫做 faults，能預料並應對故障的系統特性可稱為 fault-tolerant 或 resilient。fault-tolerant 一詞可能會產生誤導，因為它暗示著系統可以容忍所有可能的錯誤，但在實際中這是不可能的。比方說，如果整個地球（及其上的所有伺服器）都被黑洞吞噬了，想要容忍這種錯誤，需要把網路託管到太空中——這種預算能不能批准就祝你好運了。所以在討論 fault-tolerant 時，只有談論特定類型的錯誤才有意義。

注意 fault 不同於 failure【2】。Fault 通常定義為系統的一部分狀態偏離其標準，而 failure 則是整個系統停止向客戶提供服務。Fault 的機率不可能降到零，因此最好設計 fault-tolerance mechanisms 以防因 fault 而導致 failure。本書將涵蓋幾種用不可靠元件建構出可靠系統的技術。

違反直覺的是，在這類 fault-tolerant systems 中，透過故意觸發來**提高**故障率是有意義的，例如：在沒有警告的情況下隨機地殺死單一 process。許多高危漏洞實際上是由糟糕的 error handling 導致的【3】，因此我們可以透過故意引發故障來確保 fault-tolerance mechanisms 不斷執行並接受考驗，從而提高 fault 自然發生時系統能正確處理的信心。Netflix 公司的 *Chaos Monkey*【4】就是這種方法的一個例子。

儘管比起**阻止錯誤（prevent error）**，我們通常更傾向於**容忍錯誤**。但也有**預防勝於治療**的情況（比如不存在治療方法時）。安全問題就屬於這種情況。例如，如果攻擊者破壞了系統，並獲取了敏感資料，這種事是無法回頭的。但本書主要討論的是可以恢復的故障種類，正如下面幾節所述。

### Hardware Faults

當想到系統失效的原因時，硬體故障（hardware faults）總會第一個進入腦海。硬碟 crash、記憶體出錯、機房斷電、有人拔錯網線……任何與大型資料中心打過交道的人都會告訴你：一旦你擁有很多機器，這些事情**總**會發生！

有報告顯示，硬碟的 **平均無故障時間（MTTF mean time to failure）** 約為 10 到 50 年【5】【6】。因此對於一個擁有 10,000 顆硬碟的 storage cluster，我們應該預期平均每天會有 1 顆硬碟壞掉。

為了減少系統的故障率，第一個反應通常是增加單一硬體的 redundancy，例如：硬碟可以組 RAID，伺服器可能有雙路電源和熱插拔 CPU，資料中心可能有電池和柴油發電機作為後備電源，某個元件掛掉時 redundant component 可以立刻接管。這種方法雖然不能完全防止由硬體問題導致的 failures，但它簡單易懂，通常也足以讓機器不間斷執行很多年。

直到最近，硬體零件的 redundancy 對於大多數 applications 來說已經足夠了，因為它讓單台機器的 total failure 變得相當罕見。只要你能快速地把備份恢復到新機器上，對大多數 applications 而言都不算是災難性的故障停機時間。只有少數對 high availability 至關重要的 applications 才會要求 multi-machine redundancy。

但是隨著 data volumes 和 application computing demands 的增加，越來越多 applications 開始使用大量的機器，這會相應地增加 rate of hardware faults。此外在一些 cloud platforms，例如 Amazon Web Services (AWS)，virtual machine instances 變得不可用卻沒有任何警告也是很常見的【7】，因為 platforms 的設計就是優先考慮 flexibility 和 elasticity[^i]，而不是 single-machine reliability。

如果在 hardware redundancy 的基礎上進一步引入 software fault-tolerance 技術，那麼系統在 tolerate the loss of entire machines 的道路上就更進一步了。這樣的系統也有運作上的優勢，例如：如果需要重開機器（例如上 OS security patches），單一伺服器的系統就需要有計劃的停工。而能容忍 machine failure 的系統則可以一次修一個 node，不需要整個系統停工。

[^i]: 在[應對負載的方法](#應對負載的方法)一節定義

### Software Errors

我們通常認為硬體故障是隨機的、相互獨立的：一台機器的硬碟失效並不意味著另一台機器的硬碟也會失效。大量硬體元件不可能同時發生故障，除非它們存在比較弱的相關性（同樣的原因導致關聯性錯誤，例如伺服器機架的溫度）。
...
- 失控程序會用盡一些共享資源，包括CPU時間、記憶體、硬碟空間或網路頻寬。
...
- 以最小化犯錯機會的方式設計系統。例如，精心設計的抽象、API和管理後台使做對事情更容易，搞砸事情更困難。但如果介面限制太多，人們就會忽略它們的好處而想辦法繞開。很難正確把握這種微妙的平衡。
...
在[圖1-4](img/fig1-4.png)中，每個灰條代表一次對服務的請求，其高度表示請求花費了多長時間。大多數請求是相當快的，但偶爾會出現需要更長的時間的異常值。這也許是因為緩慢的請求實質上開銷更大，例如它們可能會處理更多的資料。但即使（你認為）所有請求都花費相同時間的情況下，隨機的附加延遲也會導致結果變化，例如：上下文切換到後台程序，網路資料包丟失與TCP重傳，垃圾收集暫停，強制從硬碟讀取的頁面錯誤，伺服器機架中的震動【18】，還有很多其他原因。
...
>
> 簡單的實現是在時間視窗內儲存所有請求的響應時間列表，並且每分鐘對列表進行排序。如果對你來說效率太低，那麼有一些演算法能夠以最小的CPU和記憶體成本（如前向衰減【25】，t-digest【26】或HdrHistogram 【27】）來計算百分位數的近似值。請注意，平均百分比（例如，減少時間解析度或合併來自多台機器的資料）在數學上沒有意義 - 聚合響應時間資料的正確方法是新增直方圖【28】。
...
人們經常討論**縱向伸縮（scaling up）**（**垂直伸縮（vertical scaling）**，轉向更強大的機器）和**橫向伸縮（scaling out）** （**水平伸縮（horizontal scaling）**，將負載分佈到多台小機器上）之間的對立。跨多台機器分配負載也稱為“**無共享（shared-nothing）**”架構。可以在單台機器上執行的系統通常更簡單，但高階機器可能非常貴，所以非常密集的負載通常無法避免地需要橫向伸縮。現實世界中的優秀架構需要將這兩種方法務實地結合，因為使用幾台足夠強大的機器可能比使用大量的小型虛擬機器更簡單也更便宜。
...
跨多台機器部署 **無狀態服務（stateless services）** 非常簡單，但將帶狀態的資料系統從單節點變為分散式配置則可能引入許多額外複雜度。出於這個原因，常識告訴我們應該將資料庫放在單個節點上（縱向伸縮），直到伸縮成本或可用性需求迫使其改為分散式。
...
眾所周知，軟體的大部分開銷並不在最初的開發階段，而是在持續的維護階段，包括修復漏洞、保持系統正常執行、調查失效、適配新的平台、為新的場景進行修改、償還技術債、新增新的功能等等。

不幸的是，許多從事軟體系統行業的人不喜歡維護所謂的**遺留（legacy）** 系統，——也許因為涉及修復其他人的錯誤、和過時的平台打交道，或者系統被迫使用於一些份外工作。每一個遺留系統都以自己的方式讓人不爽，所以很難給出一個通用的建議來和它們打交道。
...
- 執行復雜的維護任務，例如將應用程式從一個平台遷移到另一個平台
- 當配置變更時，維持系統的安全性
...
- 避免依賴單台機器（在整個系統繼續不間斷執行的情況下允許機器停機維護）
...
例如，高階程式語言是一種抽象，隱藏了機器碼、CPU暫存器和系統呼叫。 SQL也是一種抽象，隱藏了複雜的硬碟/記憶體資料結構、來自其他客戶端的併發請求、崩潰後的不一致性。當然在用高階語言程式設計時，我們仍然用到了機器碼；只不過沒有**直接（directly）** 使用罷了，正是因為程式語言的抽象，我們才不必去考慮這些實現細節。
...
系統的需求永遠不變，基本是不可能的。更可能的情況是，它們處於常態的變化中，例如：你瞭解了新的事實、出現意想不到的應用場景、業務優先順序發生變化、使用者要求新功能、新平台取代舊平台、法律或監管要求發生變化、系統增長迫使架構變化等。
