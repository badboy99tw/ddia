...
一個 database 在最基礎的層次上需要完成兩件事情：當你把資料交給 database 時，它應當把資料儲存起來；而後當你向 database 要資料時，它應當把資料返回給你。

在[第二章](ch2.md)中，我們討論了 data models 和 query languages，即程式設計師將資料存進 database 的 format，以及再次要回資料的機制。在本章中我們會從 database 的視角來討論同樣的問題：database 如何儲存我們提供的資料，以及如何在我們需要時重新找到資料。

作為程式設計師，為什麼要關心 database 內部如何處理 storage 和 retrieval 呢？你可能不會去從頭開始實作自己的 storage engine，但是你**確實**需要從許多可用的 storage engines 中選擇一個合適的。而且為了調整 storage engine 使之在你的工作量上運作順利，你也需要大致瞭解 storage engine 底層都在做些什麼。

特別需要注意，針對 transactional workloads 和 analytics 最佳化的 storage engines 之間存在巨大差異。稍後我們將在 [[#Transaction Processing or Analytics]] 一節中探討這一區別，並在 [[#Column-Oriented Storage]] 中討論一系列針對 analytics 最佳化的 storage engine。

但是，我們將從您最可能熟悉的兩大類資料庫：traditional relational databases 與很多所謂的 NoSQL databases 開始，透過介紹它們的 storage engines 來開始本章的內容。我們會研究兩大類 storage engines：*log-structured* storage engines，以及 *page-oriented* storage engine（例如 B-trees）。

## Data Structures That Power Your Database

世界上最簡單的 database 可以用兩個 Bash functions 實作：
...
這兩個 functions 實作了 key-value store。執行 `db_set key value` ，會將 **key** 和 **value** 存進 database。Key 和 value（幾乎）可以是你喜歡的任何東西，例如，value 可以是 JSON document。然後呼叫 `db_get key` ，查詢與該 key 關聯的最新 value 並將其返回。
...
底層的 storage format 非常簡單：一個文字檔，每行包含一條逗號分隔的 key-value pair（忽略 escaping issues 的話，大致與 CSV files 類似）。每次對 `db_set` 的呼叫都會向檔案末端追加記錄，所以更新 key 的時候舊版本的 value 不會被覆蓋 —— 因而查詢最新值的時候，需要找到檔案中 key 最後一次出現的位置（因此 `db_get` 中使用了 `tail -n 1 ` 。)
...
`db_set` function 對於極其簡單的場景其實有非常好的效能，因為在檔案尾部追加寫入通常效能是非常好的。與 `db_set` 做的事情類似，許多 databases 在內部使用了 *log*，也就是一個 append-only data file。真正的 database 有更多的問題需要處理（如 concurrency control，reclaiming disk space 以避免 log 無限增加，處理 errors 和 parttially written records），但基本原理是一樣的。Logs 極其有用，我們還將在本書的其它部分重複見到它好幾次。

> *Log* 這個詞通常指 application logs：即 application 輸出的描述發生了什麼事情的文字。本書在更普遍的意義下使用 *log* 這一詞：一個 append-only sequence of records。它不需要讓人類看得懂，it might be binary and intended only for other programs to read.

另一方面，如果這個 database 中有著大量 records，則這個 `db_get` function 的效能會非常糟糕。每次你想查詢一個 key 時，`db_get` 必須從頭到尾掃描整個 database file 來查詢 key 的出現。用演算法的語言來說，查詢花費的時間是 $O(n)$：如果 database 內的 records 數 $n$ 翻了一倍，查詢時間也要翻一倍。這就不好了。

為了有效率地找到 database 中特定 key 的值，我們需要一個不一樣的 data structure：*index*。本章將介紹一系列 indexing structures，並進行比較。它們背後的大致想法是，儲存一些額外的 metadata 作為路標，幫助你找到想要的資料。如果您想在同一份資料中以幾種不同的方式進行搜尋，那麼你也許需要不同的 indexes，建立在資料的不同部分上。

Index 是從 primary data 衍生的 *additional* structure。許多 database 允許新增與刪除 indexes，這不會影響 database 的內容，它只影響 queries 的效能。維護 additional structure 會產生開銷，特別是在寫入時。寫入效能很難超過簡單地 appending to a file，因為那是最簡單的 write operation。任何類型的 index 通常都會減慢寫入速度，因為每次寫入資料時都需要更新 index。

這是 storage systems 中一個重要的 trade-off：精心選擇的 indexes 加快了 read queries 的速度，但是每個 index 都會拖慢寫入速度。因為這個原因，database 預設並不會 index 所有的內容，而需要你（程式設計師或DBA）透過對 application's typical query patterns 的了解來手動選擇 indexes。你可以選擇能為 applicattion 帶來最大好處而且不需要引入超出必要開銷的 indexes。



### Hash Indexes

讓我們從 key-value data 的 indexes 開始。這不是您可以 index 的唯一一種類型的資料，但它很常見。對於更複雜的 indexes 來說，這是一個有用的 building block。

Key-value stores 與在大多數程式語言中可以找到的 *dictionary* type 非常相似，通常 dictionaly 都是用 hash map（或 hash table）實作。Hash maps 在許多演算法教科書中都有描述【1,2】，所以這裡我們不會討論它的細節。既然我們已經有 in-memory data sturcture —— hash maps，為什麼不使用它來 index 在 disk 上的資料呢？

假設我們的 data storage 只是 appending to a file，就像前面的例子一樣。那麼最簡單的 indexing strategy 就是：保留一個 in-memory hash map，其中每個 key 都對應到一個 data file 中的 byte offset，即可以找到 value 的位置，如 [圖3-1](img/fig3-1.png) 所示。當你 apend 一個新的 key-value pair 到檔案中，還要更新 hash map 來反映剛剛寫入的資料的 offset（這同時適用於 insert new keys 和 updating existing keys）。當你想查詢一個 value 時，使用 hash map 來查詢 data file 中的 offset，seek 到該位置並讀取 value。

![](img/fig3-1.png)

**圖3-1 以類 CSV 格式儲存 key-value paires 的 log，並使用 in-memory hash map 做 index。**

聽上去簡單，但這是一個可行的方法。現實中，Bitcask 實際上就是這麼做的（Riak 預設的 storage engine）【3】。 Bitcask 提供高效能的讀取和寫入操作，但所有 key 必須能放入可用記憶體中，因為 hash map 完全保留在記憶體中。這些 values 可以使用比可用記憶體更多的空間，因為可以從硬碟上透過一次 `seek` 讀取所需部分，如果 data file 的那部分已經在 filesystem cache 中，則讀取根本不需要任何 disk I/O。

像 Bitcask 這樣的 storage engine 非常適合每個 key 的 value 經常更新的情況。例如，key 可能是影片的 URL，value 可能是它的播放次數（每次有人點選播放按鈕時遞增）。在這種類型的 workload 中，有很多 writes，但是沒有太多 distinct keys —— 每個 key 有很多的 writes，但是將所有 keys 存在記憶體中是可行的。

直到現在，我們只是 append to a file —— 所以如何避免最終用完硬碟空間？一個好的解決方案是，將 log 分為特定大小的 segments，當 log 增加到特定大小時關閉當下的 segment file，並開始寫進新的 segment file。然後，我們就可以對這些 segments 進行 *compaction*，如 [圖3-2](img/fig3-2.png) 所示。Compaction 意味著在 log 中丟棄重複的 keys，只保留每個 key 最近的更新。

![](img/fig3-2.png)

**圖3-2 Compaction of a key-value update log（統計每一個貓影片的播放次數），只保留每個 key 最新的 value**

而且，由於 compaction 常常使得 segments 變得很小（假設在一個 segments 內 key 平均被重寫了好幾次），我們也可以在執行 compaction 的同時將多個 segments merge 在一起，如 [圖3-3](img/fig3-3.png) 所示。Segments 被寫入後永遠不會被修改，所以 merged segments 被寫進新的檔案。Frozen segments 的 merging 和 compaction 可以在 background thread 中完成，在進行時，我們仍然可以繼續使用舊的 segment files 像往常一樣提供讀寫需求。合併過程完成後，我們將讀取需求轉換為使用新的 merged segments 而不是 old segments —— 然後可以簡單地刪除 old segment files。

![](img/fig3-3.png)

**圖3-3 同時執行 compaction 和 segment merging**

每個 segment 現在都有自己的 in-memory hash table，將 key 對應到 file offsets。為了找到一個 key 的值，我們首先檢查最新的 segment 的 hash map；如果 key 不存在，我們檢查第二新的 segment，依此類推。合併過程讓 segments 維持在很小的數量，所以查詢時不需要檢查很多個 hash maps。

實作這個簡單想法時會出現大量細節。簡而言之，真正實作中幾個重要的問題是：

***File format***

CSV 不是 log 的最佳格式。使用 binary format 更快、更簡單，首先以 bytes 為單位對字串的長度進行編碼，然後使用 raw strring（不需要 escaping）。
...
如果資料庫重新啟動，則記憶體雜湊對映將丟失。原則上，您可以透過從頭到尾讀取整個段檔案並在每次按鍵時注意每個鍵的最近值的偏移量來恢復每個段的雜湊對映。但是，如果段檔案很大，這可能需要很長時間，這將使伺服器重新啟動痛苦。 Bitcask透過儲存加速恢復硬碟上每個段的雜湊對映的快照，可以更快地載入到記憶體中。
...
- 追加和分段合併是順序寫入操作，通常比隨機寫入快得多，尤其是在硬碟旋轉硬碟上。在某種程度上，順序寫入在基於快閃記憶體的 **固態硬碟（SSD）** 上也是優選的【4】。我們將在“[比較B樹和LSM樹](#比較B樹和LSM樹)”中進一步討論這個問題。
...
- 散列表必須能放進記憶體
  ...
  如果你有非常多的鍵，那真是倒黴。原則上可以在硬碟上保留一個雜湊對映，不幸的是硬碟雜湊對映很難表現優秀。它需要大量的隨機訪問I/O，當它變滿時增長是很昂貴的，並且雜湊衝突需要很多的邏輯【5】。
...
3. 由於讀取請求無論如何都需要掃描所請求範圍內的多個鍵值對，因此可以將這些記錄分組到塊中，並在將其寫入硬碟之前對其進行壓縮（如[圖3-5](img/fig3-5.png)中的陰影區域所示） 。稀疏記憶體中索引的每個條目都指向壓縮塊的開始處。除了節省硬碟空間之外，壓縮還可以減少IO頻寬的使用。
...
在硬碟上維護有序結構是可能的（請參閱“[B樹](#B樹)”），但在記憶體儲存則要容易得多。有許多可以使用的眾所周知的樹形資料結構，例如紅黑樹或AVL樹【2】。使用這些資料結構，您可以按任何順序插入鍵，並按排序順序讀取它們。

現在我們可以使我們的儲存引擎工作如下：

- 寫入時，將其新增到記憶體中的平衡樹資料結構（例如，紅黑樹）。這個記憶體樹有時被稱為**記憶體表（memtable）**。
- 當**記憶體表**大於某個閾值（通常為幾兆位元組）時，將其作為SSTable檔案寫入硬碟。這可以高效地完成，因為樹已經維護了按鍵排序的鍵值對。新的SSTable檔案成為資料庫的最新部分。當SSTable被寫入硬碟時，寫入可以繼續到一個新的記憶體表例項。
- 為了提供讀取請求，首先嚐試在記憶體表中找到關鍵字，然後在最近的硬碟段中，然後在下一個較舊的段中找到該關鍵字。
- 有時會在後台執行合併和壓縮過程以組合段檔案並丟棄覆蓋或刪除的值。

這個方案效果很好。它只會遇到一個問題：如果資料庫崩潰，則最近的寫入（在記憶體表中，但尚未寫入硬碟）將丟失。為了避免這個問題，我們可以在硬碟上儲存一個單獨的日誌，每個寫入都會立即被附加到硬碟上，就像在前一節中一樣。該日誌不是按排序順序，但這並不重要，因為它的唯一目的是在崩潰後恢復記憶體表。每當記憶體表寫出到SSTable時，相應的日誌都可以被丟棄。
...
Lucene是Elasticsearch和Solr使用的一種全文搜尋的索引引擎，它使用類似的方法來儲存它的詞典【12,13】。全文索引比鍵值索引複雜得多，但是基於類似的想法：在搜尋查詢中給出一個單詞，找到提及單詞的所有文件（網頁，產品描述等）。這是透過鍵值結構實現的，其中鍵是單詞（**關鍵詞（term）**），值是包含單詞（文章列表）的所有文件的ID的列表。在Lucene中，從術語到釋出列表的這種對映儲存在SSTable類的有序檔案中，根據需要在後台合併【14】。

#### 效能最佳化

與往常一樣，大量的細節使得儲存引擎在實踐中表現良好。例如，當查詢資料庫中不存在的鍵時，LSM樹演算法可能會很慢：您必須檢查記憶體表，然後將這些段一直回到最老的（可能必須從硬碟讀取每一個），然後才能確定鍵不存在。為了最佳化這種訪問，儲存引擎通常使用額外的Bloom過濾器【15】。 （布隆過濾器是用於近似集合內容的記憶體高效資料結構，它可以告訴您資料庫中是否出現鍵，從而為不存在的鍵節省許多不必要的硬碟讀取操作。)

還有不同的策略來確定SSTables如何被壓縮和合並的順序和時間。最常見的選擇是size-tiered和leveled compaction。LevelDB和RocksDB使用leveled compaction（LevelDB因此得名），HBase使用size-tiered，Cassandra同時支援【16】。對於sized-tiered，較新和較小的SSTables相繼被合併到較舊的和較大的SSTable中。對於leveled compaction，key範圍被拆分到較小的SSTables，而較舊的資料被移動到單獨的層級（level），這使得壓縮（compaction）能夠更加增量地進行，並且使用較少的硬碟空間。

即使有許多微妙的東西，LSM樹的基本思想 —— 儲存一系列在後台合併的SSTables —— 簡單而有效。即使資料集比可用記憶體大得多，它仍能繼續正常工作。由於資料按排序順序儲存，因此可以高效地執行範圍查詢（掃描所有高於某些最小值和最高值的所有鍵），並且因為硬碟寫入是連續的，所以LSM樹可以支援非常高的寫入吞吐量。
...
我們前面看到的日誌結構索引將資料庫分解為可變大小的段，通常是幾兆位元組或更大的大小，並且總是按順序編寫段。相比之下，B樹將資料庫分解成固定大小的塊或頁面，傳統上大小為4KB（有時會更大），並且一次只能讀取或寫入一個頁面。這種設計更接近於底層硬體，因為硬碟也被安排在固定大小的塊中。
...
如果要更新B樹中現有鍵的值，則搜尋包含該鍵的葉頁，更改該頁中的值，並將該頁寫回到硬碟（對該頁的任何引用保持有效） 。如果你想新增一個新的鍵，你需要找到其範圍包含新鍵的頁面，並將其新增到該頁面。如果頁面中沒有足夠的可用空間容納新鍵，則將其分成兩個半滿頁面，並更新父頁面以解釋鍵範圍的新分割槽，如[圖3-7](img/fig3-7.png)所示[^ii]。
...
B樹的基本底層寫操作是用新資料覆蓋硬碟上的頁面。假定覆蓋不改變頁面的位置：即，當頁面被覆蓋時，對該頁面的所有引用保持完整。這與日誌結構索引（如LSM樹）形成鮮明對比，後者只附加到檔案（並最終刪除過時的檔案），但從不修改檔案。
...
為了使資料庫對崩潰具有韌性，B樹實現通常會帶有一個額外的硬碟資料結構：**預寫式日誌（WAL, write-ahead-log）**（也稱為**重做日誌（redo log）**）。這是一個僅追加的檔案，每個B樹修改都可以應用到樹本身的頁面上。當資料庫在崩潰後恢復時，這個日誌被用來使B樹恢復到一致的狀態【5,20】。

更新頁面的一個額外的複雜情況是，如果多個執行緒要同時訪問B樹，則需要仔細的併發控制 —— 否則執行緒可能會看到樹處於不一致的狀態。這通常透過使用**鎖存器（latches）**（輕量級鎖）保護樹的資料結構來完成。日誌結構化的方法在這方面更簡單，因為它們在後台進行所有的合併，而不會干擾傳入的查詢，並且不時地將舊的分段原子交換為新的分段。
...
- 通常，頁面可以放置在硬碟上的任何位置；沒有什麼要求附近的鍵放在頁面附近的硬碟上。如果查詢需要按照排序順序掃描大部分關鍵字範圍，那麼這種按頁面儲存的佈局可能會效率低下，因為每個讀取的頁面都可能需要硬碟尋道。因此，許多B樹實現嘗試佈局樹，使得葉子頁面按順序出現在硬碟上。但是，隨著樹的增長，維持這個順序是很困難的。相比之下，由於LSM樹在合併過程中一次又一次地重寫儲存的大部分，所以它們更容易使順序鍵在硬碟上彼此靠近。
- 額外的指標已新增到樹中。例如，每個葉子頁面可以在左邊和右邊具有對其兄弟頁面的引用，這允許不跳回父頁面就能順序掃描。
- B樹的變體如分形樹【22】借用一些日誌結構的思想來減少硬碟尋道（而且它們與分形無關）。
...
由於反覆壓縮和合並SSTables，日誌結構索引也會重寫資料。這種影響 —— 在資料庫的生命週期中每次寫入資料庫導致對硬碟的多次寫入 —— 被稱為**寫放大（write amplification）**。需要特別注意的是固態硬碟，固態硬碟的快閃記憶體壽命在覆寫有限次數後就會耗盡。

在寫入繁重的應用程式中，效能瓶頸可能是資料庫可以寫入硬碟的速度。在這種情況下，寫放大會導致直接的效能代價：儲存引擎寫入硬碟的次數越多，可用硬碟頻寬內的每秒寫入次數越少。
...
LSM樹可以被壓縮得更好，因此經常比B樹在硬碟上產生更小的檔案。 B樹儲存引擎會由於分割而留下一些未使用的硬碟空間：當頁面被拆分或某行不能放入現有頁面時，頁面中的某些空間仍未被使用。由於LSM樹不是面向頁面的，並且定期重寫SSTables以去除碎片，所以它們具有較低的儲存開銷，特別是當使用分層壓縮（leveled compaction）時【27】。
...
日誌結構儲存的缺點是壓縮過程有時會干擾正在進行的讀寫操作。儘管儲存引擎嘗試逐步執行壓縮而不影響併發訪問，但是硬碟資源有限，所以很容易發生請求需要等待硬碟完成昂貴的壓縮操作。對吞吐量和平均響應時間的影響通常很小，但是在更高百分比的情況下（請參閱“[描述效能](ch1.md#描述效能)”），對日誌結構化儲存引擎的查詢響應時間有時會相當長，而B樹的行為則相對更具可預測性【28】。

壓縮的另一個問題出現在高寫入吞吐量：硬碟的有限寫入頻寬需要在初始寫入（記錄和重新整理記憶體表到硬碟）和在後台執行的壓縮執行緒之間共享。寫入空資料庫時，可以使用全硬碟頻寬進行初始寫入，但資料庫越大，壓縮所需的硬碟頻寬就越多。

如果寫入吞吐量很高，並且壓縮沒有仔細配置，壓縮跟不上寫入速率。在這種情況下，硬碟上未合併段的數量不斷增加，直到硬碟空間用完，讀取速度也會減慢，因為它們需要檢查更多段檔案。通常情況下，即使壓縮無法跟上，基於SSTable的儲存引擎也不會限制傳入寫入的速率，所以您需要進行明確的監控來檢測這種情況【29,30】。
...
本章到目前為止討論的資料結構都是對硬碟限制的回答。與主記憶體相比，硬碟處理起來很尷尬。對於硬碟和SSD，如果要在讀取和寫入時獲得良好效能，則需要仔細地佈置硬碟上的資料。但是，我們容忍這種尷尬，因為硬碟有兩個顯著的優點：它們是持久的（它們的內容在電源關閉時不會丟失），並且每GB的成本比RAM低。

隨著RAM變得更便宜，每GB成本的論據被侵蝕了。許多資料集不是那麼大，所以將它們全部儲存在記憶體中是非常可行的，可能分佈在多個機器上。這導致了記憶體資料庫的發展。

某些記憶體中的鍵值儲存（如Memcached）僅用於快取，在重新啟動計算機時丟失的資料是可以接受的。但其他記憶體資料庫的目標是永續性，可以透過特殊的硬體（例如電池供電的RAM），將更改日誌寫入硬碟，將定時快照寫入硬碟或者將記憶體中的狀態複製到其他機器上。

記憶體資料庫重新啟動時，需要從硬碟或透過網路從副本重新載入其狀態（除非使用特殊的硬體）。儘管寫入硬碟，它仍然是一個記憶體資料庫，因為硬碟僅出於永續性目的進行日誌追加，讀取完全由記憶體提供。寫入硬碟也具有運維優勢：硬碟上的檔案可以很容易地由外部實用程式進行備份，檢查和分析。

諸如VoltDB，MemSQL和Oracle TimesTen等產品是具有關係模型的記憶體資料庫，供應商聲稱，透過消除與管理硬碟上的資料結構相關的所有開銷，他們可以提供巨大的效能改進【41,42】。 RAM Cloud是一個開源的記憶體鍵值儲存器，具有永續性（對儲存器中的資料以及硬碟上的資料使用日誌結構化方法）【43】。 Redis和Couchbase透過非同步寫入硬碟提供了較弱的永續性。

反直覺的是，記憶體資料庫的效能優勢並不是因為它們不需要從硬碟讀取的事實。即使是基於硬碟的儲存引擎也可能永遠不需要從硬碟讀取，因為作業系統在記憶體中快取了最近使用的硬碟塊。相反，它們更快的原因在於省去了將記憶體資料結構編碼為硬碟資料結構的開銷。【44】。

除了效能，記憶體資料庫的另一個有趣的領域是提供難以用基於硬碟的索引實現的資料模型。例如，Redis為各種資料結構（如優先順序佇列和集合）提供了類似資料庫的介面。因為它將所有資料儲存在記憶體中，所以它的實現相對簡單。
...
為了處理像[例3-1]()這樣的查詢，您可能在 `fact_sales.date_key`， `fact_sales.product_sk`上有索引，它們告訴儲存引擎在哪裡查詢特定日期或特定產品的所有銷售情況。但是，面向行的儲存引擎仍然需要將所有這些行（每個包含超過100個屬性）從硬碟載入到記憶體中，解析它們，並過濾掉那些不符合要求的屬性。這可能需要很長時間。
...
這個想法的巧妙擴充套件在C-Store中引入，並在商業資料倉庫Vertica【61,62】中被採用。不同的查詢受益於不同的排序順序，為什麼不以幾種不同的方式來儲存相同的資料呢？無論如何，資料需要複製到多台機器，這樣，如果一台機器發生故障，您不會丟失資料。您可能還需要儲存以不同方式排序的冗餘資料，以便在處理查詢時，可以使用最適合查詢模式的版本。
...
日誌結構的儲存引擎是相對較新的發展。他們的主要想法是，他們系統地將隨機訪問寫入順序寫入硬碟，由於硬碟驅動器和固態硬碟的效能特點，可以實現更高的寫入吞吐量。在完成OLTP方面，我們透過一些更復雜的索引結構和為保留所有資料而最佳化的資料庫做了一個簡短的介紹。

然後，我們從儲存引擎的內部繞開，看看典型資料倉庫的高階架構。這一背景說明了為什麼分析工作負載與OLTP差別很大：當您的查詢需要在大量行中順序掃描時，索引的相關性就會降低很多。相反，非常緊湊地編碼資料變得非常重要，以最大限度地減少查詢需要從硬碟讀取的資料量。我們討論了列式儲存如何幫助實現這一目標。
